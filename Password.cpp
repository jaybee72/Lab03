	...
	for (int i = 0; i < len; i++)
		{
			...
			if (compare_string->charAt(i) == all_words->get(try_password)->charAt(i))
				{
					matches++;
				}
		}
...
{
	ListArrayIterator<String>* guess_iterator = all_words->iterator();
	
	int matches = 0; //the number of matches of the characters found between the two strings being compared at one time
	
	
	
	//for (int i = 0; i < all_words->size; i++)
	while (guess_iterator->hasNext())
		{
			String* nextItem = guess_iterator->next;
			matches = getNumMatches(nextItem, all_words->get(try_password);
			/*
			for (int i = 0; i < len; i++)
				{
					...
					if (compare_string->charAt(i) == all_words->get(try_password)->charAt(i))
						{
							matches++;
							//viable_words->add(all_words->get(try_password))
						}
				}
			*/
			...
			if (matches == num_matches)
				{
					viable_words->add(nextItem);
				}
		}
		
	delete guess_iterator;
}
...
{
	ListArrayIterator<String>* display_iterator;
	
	for (int i = 0; i < viable_words->size; i++)
	{
		displayiterator->next->displayString;
	}
}
